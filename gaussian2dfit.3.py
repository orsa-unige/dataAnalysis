...
class input_parameters(object):      # Mettere tutto il JSON in un oggetto Python
...
bell=models.Gaussian2D(amplitude=1200, x_mean=p.x, y_mean=p.y, x_stddev=6, y_stddev=6, theta=0.3 )
back = models.Polynomial2D(degree=0) # Altro modello per il fondo (una costante)

f_init = bell+back # Sommo bellamente i due modelli (vedere compound model nella pagina pyastropd)

# # Per il futuro: un modello migliore per le PSF (la gaussiana bivariata fa schifo)
